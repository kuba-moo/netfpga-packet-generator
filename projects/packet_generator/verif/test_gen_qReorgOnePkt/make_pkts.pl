...
# Reset the SRAM queue sizes

my $defaultQSize = 0x80000 / 8;
my $xmitQSize = 4096;
my $rcvQSize = $defaultQSize - $xmitQSize;
...
for (my $i = 0; $i < 4; $i++) {
	...
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_LO_REG + ($i * 2) * $qAddrOffset,
		$defaultQSize * $i);
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_HI_REG + ($i * 2) * $qAddrOffset,
		$defaultQSize * $i + $xmitQSize - 1);
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_CTRL_REG() + ($i * 2) * $qAddrOffset,
		0x03);


	# Set queue sizes for queues 1, 3, 5 and 7
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_LO_REG + ($i * 2 + 1) * $qAddrOffset,
		$defaultQSize * $i + $xmitQSize);
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_HI_REG + ($i * 2 + 1) * $qAddrOffset,
		$defaultQSize * ($i + 1) - 1);
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_CTRL_REG() + ($i * 2 + 1) * $qAddrOffset,
		0x03);


	# Set queue sizes for queues 8, 9, 10, 11
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_LO_REG + ($i + 8) * $qAddrOffset,
		$defaultQSize * ($i + 4) );
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_ADDR_HI_REG + ($i + 8) * $qAddrOffset,
		$defaultQSize * ($i + 4 + 1) - 1);
	nf_PCI_write32($delay, $batch,
		OQ_QUEUE_0_CTRL_REG() + ($i + 8) * $qAddrOffset,
		0x03);
...
nf_PCI_write32($delay, $batch, OQ_QUEUE_0_ADDR_HI_REG() + 8 * $qAddrOffset, scalar(@paddedPkt) / 8 + $defaultQSize * 4);
