...
my $rateLimOffset = RATE_LIMIT_OFFSET();
my $rateLimBase = RATE_LIMIT_0_BASE_ADDR(); #ENABLE_REG() + (0x10 - 4) * $rateLimOffset;
...
nf_PCI_write32($delay, $batch, RATE_LIMIT_0_TOKEN_INTERVAL_REG(), 8);
nf_PCI_write32($delay, $batch, RATE_LIMIT_0_TOKEN_INC_REG(), 2);
nf_PCI_write32($delay, $batch, RATE_LIMIT_0_CTRL_REG(), 1);
...
nf_PCI_write32($delay, $batch, OQ_QUEUE_0_MAX_ITER_REG() + 8 * $qAddrOffset, 2);
