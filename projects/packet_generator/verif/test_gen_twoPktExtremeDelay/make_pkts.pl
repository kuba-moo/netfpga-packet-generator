...
# Write the header to SRAM -- length
nf_PCI_write32($delay, $batch, $addr + 0x4, 0x000000ff);
nf_PCI_write32($delay, $batch, $addr + 0x8, $wordLen | ($dstPort << 16));
nf_PCI_write32($delay, $batch, $addr + 0xc, $len | ($srcPort << 16));
$addr += 0x10;

# Write the header to SRAM -- delay
nf_PCI_write32($delay, $batch, $addr + 0x4, DELAY_CTRL_VAL());
nf_PCI_write32($delay, $batch, $addr + 0x8, 0x00000000);
nf_PCI_write32($delay, $batch, $addr + 0xc, 0x00000000);
$addr += 0x10;
...
nf_PCI_write32($delay, $batch, $addr + 0x4, DELAY_CTRL_VAL());
nf_PCI_write32($delay, $batch, $addr + 0x8, 0x00000000);
nf_PCI_write32($delay, $batch, $addr + 0xc, 0x0fffffff);
$addr += 0x10;
...
nf_PCI_write32($delay, $batch,
	...
	(scalar(@paddedPkt) / 8 + 1) * 2 + 1 + 8 * $defaultQSize);
...
# Expect the packet
#
# Note: expect only one since we have such an extreme delay between the packets
# We should only see one packet come out...
