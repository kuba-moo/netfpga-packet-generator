...
// $Id: mac_grp_regs.v 3132 2007-12-13 00:27:33Z jnaous $
//
...
//         else if (!new_reg_req && reg_cnt == `TX_QUEUE_NUM_PKTS_ENQUEUED)
...
module mac_grp_regs
   ...
   ( input                                mac_grp_reg_req,
     ...
     // interface to time stamp
     input [31:0]                         Time_LO_tx,
     input [31:0]                         Time_HI_tx,
     input                                valid_tx,
     input [31:0]                         Time_LO_rx,
     input [31:0]                         Time_HI_rx,
     input                                valid_rx,
   ...
   // ------------- Internal parameters --------------
   localparam NUM_REGS_USED         = 20; /* don't forget to update this when adding regs */
...
/* -----\/----- EXCLUDED -----\/-----
      if (valid_tx)
	begin
	   reg_file[`CLK_SYN_TX_LO]    <= Time_LO_tx;
	   reg_file[`CLK_SYN_TX_HI]    <= Time_HI_tx;
	end
      if (valid_rx)
	begin
	   reg_file[`CLK_SYN_RX_LO]    <= Time_LO_rx;
	   reg_file[`CLK_SYN_RX_HI]    <= Time_HI_rx;
       end
      reg_file[`CLK_SYN_TX_VALID] <= valid_tx;
      reg_file[`CLK_SYN_RX_VALID] <= valid_rx;
 -----/\----- EXCLUDED -----/\----- */
   end
